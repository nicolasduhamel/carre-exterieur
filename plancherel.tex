...
\begin{proof}
Il suffit de montrer la convegence de l'integrale
\begin{equation}
...
\end{equation}
pour tout $k \in K_n$. On effectue la meme majoration que pour la convergence de l'integrale $J(s,W,\phi)$, l'integrale est donc majoree par
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
pour tout $N \geq 1$. Cette derniere integrale est convergente pour $N$ suffisament grand par le meme argument que pour la convergence de $J(s,W,\phi)$.

Passons a la deuxieme partie, comme $\pi = T(\sigma)$, on sait que $\pi \simeq \widetilde{\pi}$ donc $\pi$ est $(H_n, \theta)$-distinguee. Autrement dit, $Hom_{H_n}(\pi, \theta) \neq 0$. Ce dernier est un sous-espace de $Hom_{H_n \cap P_{2n}}(\pi, \theta)$ qui est de dimension au plus $1$ (Matringe, Prop 4.3). On en deduit que la restiction de $\beta$ a $\mathcal{W}(\pi, \psi)$, qui est bien $H_n \cap P_{2n}$-invariant, est un element de $Hom_{H_n}(\pi, \theta)$.

Pour finir, le modele de Kirillov $\mathcal{K}(\pi, \psi)$ contient $C^\infty_c(N_{2n} \backslash P_{2n}, \psi)$ (Gelfand-Kazhdan). En particulier, il existe une fonction de Whittaker dont la restriction a $A_{n-1}K_n$ est l'indicatrice de $A_{n-1}(\mathcal{O}_F)$, alors $\beta$ est non nulle sur cette fonction.
\end{proof}
...
\end{equation}
pour tout $\pi \in T(Temp(SO(2n+1)))$. La quantite $(\varphi_1, \varphi_2)_{Y_n, \pi}$ est independante du choix de $f_1,f_2$. Alors on a
\begin{equation}
...
\begin{proof}
La fonction $\xi_p \mapsto \beta\left(W_{f,T(\sigma)}(\xi_p,.)\right)$ est lisse donc a support compact. De plus,
\end{proof}
...
Montrons que la quantite, $(\varphi_1, \varphi_2)_{Y_n, \pi}$ est independante du choix de $f_1, f_2$. Commencons par le
\begin{lemme}
\label{decbase}
On note $\pi = T(\sigma)$ avec $\sigma \in Temp(SO(2n+1))$. L'operateur $\pi(f^{\check{}}) : \mathcal{W}(\pi, \psi) \rightarrow \mathcal{W}(\pi, \psi)$ est de rang fini. Notons $\mathcal{B}(\pi, \psi)_f$ une base fini orthonormee de son image. Alors
\begin{equation}
...
Le lemme \ref{decbase} donne que
\begin{equation}
(\varphi_1, \varphi_2)_{Y_n, T(\sigma)} = \sum_{W' \in \mathcal{B}(\pi, \psi)_f} \overline{\beta(\pi(f_2)W')} \otimes \beta(\pi(f_1)W');
\end{equation}
qui est bien independant du choix de $f_1,f_2$ puisque la restriction de $\beta$ a $\mathcal{W}(\pi, \psi)$ est $H_n$ invariante.
