...
Pour finir, le modèle de Kirillov $\mathcal{K}(\pi, \psi)$ contient $C^\infty_c(N_{2n} \backslash P_{2n}, \psi)$ (Gelfand-Kazhdan). En particulier, il existe une fonction de Whittaker dont la restriction a $A_{n-1}K_n$ est l'indicatrice de $A_{n-1}(\mathcal{O}_F)$, alors $\beta$ est non nulle sur cette fonction.
\end{proof}
...
\begin{lemme}
\label{decbase}
On note $\pi = T(\sigma)$ avec $\sigma \in Temp(SO(2n+1))$. L'opérateur $\pi(f^{\check{}}) : \mathcal{W}(\pi, \psi) \rightarrow \mathcal{W}(\pi, \psi)$ est de rang fini. Notons $\mathcal{B}(\pi, \psi)_f$ une base finie orthonormée de son image. Alors
\begin{equation}
