...
\begin{lemme}
Pour $W \in C^w(N_{2n}(F)\backslash{GL_{2n}(F)}, \psi)$ et $\phi \in \mathcal{S}(F^n)$, l'intégrale $J(s, W, \phi)$ converge absolument pour tout $s \in \mathbb{C}$ vérifiant $Re(s) > 1$.
\end{lemme}
 
 \begin{proof}
 D'après la décomposition d'Iwasawa, on a $N_n\backslash{G_n} = A_nK_n$. Il suffit de montrer la convergence de l'intégrale
 \begin{equation}
 \int_{A_n} \int_{K_n} \int_{Lie(B_n)\backslash{M_n}} |W\left(\sigma \begin{pmatrix}
1 & X \\
0 & 1
\end{pmatrix}\begin{pmatrix}
ak & 0 \\
0 & ak
\end{pmatrix}\right)| dX dk |\det a|^{Re(s)} \delta^{-1}(a) da.
 \end{equation}
...
\end{equation}
 ...
 \begin{equation}
\int_{A_n} \int_{K_n} \int_{Lie(B_n)\backslash{M_n}} |W\left(b u_X \sigma \begin{pmatrix}
k & 0 \\
0 & k
\end{pmatrix}\right)|dX dk |\det a|^{Re(s)} \delta^{-2}(a) da.
 \end{equation}
...
\end{pmatrix}$. Le lemme \ref{majtemp} donne alors
 ...
 et
 \begin{equation}
 \int_{A_n}  \prod (1+ |\frac{a_i}{a_{i+1}}|)^{-N} \log(||b||)^d|\det a|^{Re(s)} \delta^{\frac{1}{2}}(b)\delta^{-2}(a) da.
 \end{equation}
