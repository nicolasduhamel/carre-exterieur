...
Soit $\theta$ le caractère sur $H_n$ défini par $\psi(Tr(X))$.
...
\begin{theoreme}
Soit $F$ un corps $p$-adique. Soit $\varphi_1, \varphi_2 \in \mathcal{S}(H_n(F) \backslash GL_{2n}(F), \theta)$. On a l'égalité
\begin{equation}
(\phi_1, \phi_2)_{L^2(H_n(F) \backslash G_{2n}(F))} = \int_{Temp(SO(2n+1))/Stab} (\varphi_1, \varphi_2)_{H_n(F) \backslash GL_{2n}(F), T(\sigma)} \frac{|\gamma^*(0, \sigma, Ad, \psi)|}{|S_\sigma|}d\sigma,
\end{equation}
où $T : Temp(SO(2n+1))/Stab \rightarrow Temp(G_{2n})$ est l'application de transfert, $(\varphi_1, \varphi_2)_{H_n(F) \backslash G_{2n}(F), T(\sigma)}$ est défini dans le théorème \ref{thPlanch} à partir de la forme linéaire $\beta$.
\end{theoreme}

\begin{theoreme}
Soit $F$ un corps $p$-adique. Une décomposition de Plancherel abstraite sur $L^2(GL_n(F) \times GL_n(F) \backslash GL_{2n}(F))$ est obtenue par la relation
\begin{equation}
L^2(GL_n(F) \times GL_n(F) \backslash GL_{2n}(F)) = \int^{\oplus}_{Temp(SO(2n+1)(F))/Stab} T(\sigma) \frac{|\gamma^*(0, \sigma, Ad, \psi)|}{|S_\sigma|} d\sigma.
\end{equation}
\end{theoreme}

\subsection{Notations}
Dans la suite on notera $F$ un corps $p$-adique (sauf dans la section \ref{gamma} où $F$ peut désigner un corps archimédien) et $\psi$ un caractère non trivial de $F$. On note $G_{2n}$ le groupe $GL_{2n}(F)$. On notera dans la suite $H_n=H_n(F)$ le groupe des $F$-points. On note $B_n$ le sous groupe des matrices triangulaires supérieures, $N_n$ le sous-groupe de $B_n$ des matrices dont les éléments diagonaux sont $1$ et $M_n$ l'ensemble des matrices de taille $n \times n$ à coefficients dans $F$. On note $U_n$ le groupe des matrices de la forme $\begin{pmatrix}
1_{n-1} & x \\
0 & 1
\end{pmatrix}$ pour $x \in F^{n-1}$ et $P_n = G_{n-1}U_n$ le sous-groupe mirabolique.

Soit $G$ un groupe réductif connexe (dans la suite $G$ sera $GL_{2n}$, $SO_{2n+1}$ ou un quotient, sous-groupe de Levi de ces groupes). On note $Temp(G)$ l'ensemble des classes d'isomorphismes de représentations irréductibles tempérées de $G(F)$. On note $Z_G$ le centre de $G(F)$ et $A_G$ le tore déployé maximal dans $Z_G$.

\section{Facteurs $\gamma$ du carré extérieur}
\label{gamma}
Dans cette partie $F$ un corps local de caractéristique $0$ et $\psi$ un caractère non trivial de $F$. Soit $\pi$ une représentation tempérée irréductible de $GL_{2n}(F)$. Jacquet et Shalika ont défini une fonction L du carré extérieur $L_{JS}(s, \pi, \Lambda^2)$ par des intégrales notées $J(s, W, \phi)$, où $W \in \mathcal{W}(\pi, \psi)$ est un élément du modèle de Whittaker de $\pi$ et $\phi \in \mathcal{S}(F^n)$ est une fonction de Schwartz. Matringe a prouvé que, lorsque $F$ est non archimédien, ces intégrales $J(s,W,\phi)$ vérifient une équation fonctionnelle, ce qui permet de définir des facteurs $\gamma$, que l'on note $\gamma^{JS}(s,\pi,\Lambda^2,\psi)$. 
